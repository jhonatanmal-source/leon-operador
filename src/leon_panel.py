    ...
    function renderOperators(status) {
      ...
      const html = [
        ...
        renderOperator("Professor LEON", operators.professor, [
          ...
          ["Blocos revisados", status.batch_learning?.last_batch_number ?? status.batch_learning?.processed_batches?.length ?? 0],
          ["Operações válidas", status.batch_learning?.closed_operations_in_window ?? status.batch_learning?.closed_operations ?? 0],
          ["Janela (dias)", status.batch_learning?.window_days ?? "-"],
