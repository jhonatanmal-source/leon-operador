...
- **Telegram REATIVADO (2026-08-18)**: token real do BotFather + chat_id configurados em `/opt/leon/app/.env` (`LEON_TELEGRAM_TOKEN`, `LEON_TELEGRAM_CHAT_ID`, `LEON_TELEGRAM_ENABLED=true`) — arquivo lido pelo runtime (`telegram_config.py` lê `ROOT_DIR/.env` + `config.ini` como fallback). `config.ini` `[TELEGRAM] enabled = true`. **O `/opt/leon/config/.env` NÃO é lido pelo runtime** (contém placeholder antigo `COLAR_...` — não criar symlink `app/.env -> config/.env`, pois sobrescreveria as chaves web reais do `app/.env` e o placeholder seria lido como token). Envio real validado: `message_id 3364` no grupo "LEON XAU AI - Estudos". Para desativar: `config.ini` `enabled = false` (guard `TELEGRAM_DISABLED`).
